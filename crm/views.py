...
def summary(request, pk):
    ...
    #sum_total_charge = Product.objects.filter(cust_name=pk).aggregate(Sum('charge')) + Service.objects.filter(cust_name=pk).aggregate(Sum('service_charge'))
    ...
    #sum = sum_total_charge.get("service_charge__sum")
    #if sum== None:
        #sum_total_charge = {'service_charge__sum' : Decimal('0')}
